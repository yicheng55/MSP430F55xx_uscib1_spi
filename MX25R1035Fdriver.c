...
SST25VF020B 2 Mbit Serial Flash Memory

March 8, 2010, Rev. 1.0

ABOUT THE SOFTWARE
This application note provides software driver examples for SST25VF020B,
...
ABOUT THE SST25VF020B

Companion product datasheets for the SST25VF020B should be reviewed in 
...
Note:  The pin names of the SST25VF020B are used in this application note. The associated test code
...
Read_Status_Register1			Reads the status register1 of the serial flash
EWSR					Enables the Write Status Register
WRSR					Performs a write to the Status Register or Status Register 1
...
/********************************************************************/
/* Copyright Silicon Storage Technology, Inc. (SST), 1994-2010	    */
/* Example "C" language Driver of SST25VF020B Serial Flash	    */
/* Hardik Patel, Silicon Storage Technology, Inc.                  */
/*                                                                  */
/* Revision 1.0, March 8, 2010			  	    */   
/*                                                                  */
/*								    */
/********************************************************************/
...
/*									*/
/************************************************************************/
void CE_High() 			//disable chip through
{
	//CE = 1;				/* set CE high */
	P4OUT &= ~CE;
	P4OUT |= CE;
...
/*									*/
/************************************************************************/
void CE_Low() 			//enable chip through
{	
	//CE = 0;				/* clear CE low */
	P4OUT |= CE;
	P4OUT &= ~CE;
...
/************************************************************************/
/* PROCEDURE: Read_Status_Register1					*/
/*									*/
/* This procedure read the status register1 and returns the byte.	*/
/*									*/
/* Input:								*/
/*		None							*/
/*									*/
...
unsigned char Read_Status_Register1()
{
	unsigned char byte = 0;
	CE_Low();			/* enable device */
	Send_Byte(0x35);		/* send RDSR1 command */
	byte = Get_Byte();		/* receive byte */
	CE_High();			/* disable device */
	return byte;
}
...
/* PROCEDURE: WRSR							*/
/*									*/
/* This procedure writes to the Status Register and Status Register 1.			*/
/*									*/
/* Input:								*/
/*		byte							*/
/*									*/
...
void WRSR(unsigned long word, unsigned char byteOrword)
{
	...
	if (byteOrword ==1) 	  // write to status register
	{Send_Byte(word & 0xff);}		/* data that will change the status of BPx 
					   or BPL (only bits 2,3,4,5,7 can be written) */
	else
	{Send_Byte(word & 0xff);	   // write to status register and status register 1
...
/* and device ID (8Ch).  It will use 9Fh as the JEDEC ID command.    	*/
...
unsigned long Jedec_ID_Read() 
{
	...
	temp = (temp | Get_Byte()); 	 /* temp value = 0xBF258C */
	CE_High();			 /* disable device */
...
/*									*/
/* Input:								*/
/*		Dst:	Destination Address 000000H - 3FFFH		*/
/*      								*/
/*									*/
/* Returns:								*/
/*		byte							*/
